---
title: "Benchmark methods Part 1: Application to one Time Series"
...
theta_classic <- Theta.classic(seas_adj(train)$des_input, fh=give_fh(train))$mean*seas_adj(train)$SIout

# vs built-in theta method
theta_built_in <- thetaf(train, h=give_fh(train))$mean

autoplot(theta_classic, serie=) +
  autolayer(theta_built_in)
```

Both methods are the same. For simplicity, we'll use the built-in function `thetaf()` from here. 
