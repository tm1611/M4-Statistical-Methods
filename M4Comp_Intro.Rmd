...
- **Naive** (`f1`): Naive method using using `naive(input, h=fh)$mean` resulting in a point forecast 
- **Seasonal Naive** (`f2`) : Seasonal naive method using `snaive(input, h=fh)$mean`
- **Naive2** (`f3`): Naive method applied on deseasonalized input using `naive(des_input, h=fh)$mean`
- **Simple exponential smoothing** (`f4`): SES method using `ses(des_input, h=fh)$mean` 
