...
# mean difference to des
mean(abs(train - des_train), na.rm=TRUE)
```

## Benchmark methods

The M4 competition includes 8 benchmark methods. This document follows the notation enumerating them from `f1` to `f8`. `h` indicates the frequency-specific forecast horizon which is given by `give_fh(input)`. In general, `input` indicates the time series in the original form and `des_input` indicates the deseasonalized version of the series using `seas_adj(input)$des_input`. `SIout` gives the seasonal component which is added to the deseasonalized forecast and can be obtained using `seas_adj(input)$SIout`:

- `f1`: **Naive** method using `naive(input, h=fh)$mean` resulting in a point forecast 
- `f2`: **Seasonal naive** method using `snaive(input, h=fh)`
- `f3`: **Naive2**, naive method applied on deseasonalized input using `naive(des_input, h=fh)$mean`
- `f4`: **Simple exponential smoothing** using `ses(des_input) 
