...
To decompose a time series into seasonal, trend and irregular components the **classical decomposition** is used based on moving averages. In R, the decomposition can be calculated using the `decompose()` function.
...
In classical decomposition the seasonal component is assumed to be constant, whereas for multiplicative seasonality the seasonality is varying. In the M4 competition the "multiplicative" version is used, which will therefore be explained here. The additive decomposition, however, works similar. 
