...
for (i in 1:length(df)){
  ...
  if(i%%10 ==0){
    pct <- round((i/n)*100,4)
    print(noquote(paste0(i, "/",length(df), " - " ,pct, "%")))
  }
  if(i%%n==0){
    print("Done!")
  }
...
res_bm8_quarterly <- data.frame(Series=sn, sMAPE=Total_sMAPE, MASE=Total_MASE)
write.csv(res_bm8_quarterly, file="results/benchmarks/results_bm8_quarterly.csv")

res_table_bm8_quarterly <- my_accuracy(Total_sMAPE, Total_MASE)
write.csv(res_table_bm8_quarterly, file="results/benchmarks/results_bm8_quarterly_table.csv")
