...
# load data
my_data <- "data/M4_Industry.rds"
df <- give_sam(readRDS(file=my_data),size = 1880, seed = 16) #10%
...
results_Industry <- data.frame(Series=st, insample_n=n, period=period, sMAPE=Total_sMAPE, MASE=Total_MASE)
write.csv(results_Industry, file="results/M4_ETSARIMA/results_Industry_1000.csv")

results_table_Industry <- my_accuracy(Total_sMAPE, Total_MASE)
write.csv(results_table_Industry, file="results/M4_ETSARIMA/results_Industry_1000_table.csv")
