...
error_deepar_daily <- read.csv("data/m4_daily_id_deepar_error_metrics_owa1014.csv")
...
summary(lm(formula = MASE ~ n + type + Entropy + Trend + ACF1 + Lambda, data=df_processed))
summary(lm(formula = sMAPE ~ n + type + Entropy + Trend + ACF1 + Lambda, data=df_processed))
